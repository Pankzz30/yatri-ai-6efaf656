...
const MyTrips = () => {
  ...
  const saved = destinations.filter((d) => user?.savedTrips.includes(d.id));
    ...
    return (
      <div className="flex min-h-screen items-center justify-center pt-16">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-xl font-bold text-foreground">Sign in to view trips</h2>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {saved.length === 0 ? (
            <div className="mt-16 text-center">
              <MapPin size={48} className="mx-auto text-muted-foreground" />
              <p className="mt-4 text-muted-foreground">No saved trips yet. Start exploring!</p>
            ...
            <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {saved.map((d, i) => <DestinationCard key={d.id} destination={d} index={i} />)}
            </div>
          )}
        </motion.div>
      </div>
