        ...
        {sent ? (
          <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="text-center">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-accent">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.15, type: "spring", stiffness: 300, damping: 20 }}>
          ...
          <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.35 }}>
            <Link to="/login" className="mb-8 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
              <ArrowLeft size={14} /> Back to sign in
            </Link>

            <div className="mb-8">
              <h1 className="text-[1.85rem] font-bold text-foreground leading-tight">Forgot password?</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-1">
                <AuthButton loading={loading} loadingText="Sending link...">
                  <span className="flex items-center justify-center gap-2"><Send size={15} /> Send reset link</span>
                </AuthButton>
              </div>
            </form>
          </motion.div>
